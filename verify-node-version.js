...
function checkNodeVersion() {
  ...
  if (currentMajor === requiredMajor) {
    log(`✅ Perfect! You're using Node ${currentVersion} which matches the required version.`, 'green');
    return true;
  } else {
    log(`\n❌ ${colors.bold}INCOMPATIBLE NODE.JS VERSION DETECTED${colors.reset}`, 'red');
    log(`   Required: Node ${requiredVersion}`, 'red');
    log(`   Current:  ${currentVersion}`, 'red');
    log(`\n⚠️  ${colors.bold}Why this matters:${colors.reset}`, 'yellow');
    log(`   • Using an incompatible Node version will cause the build to fail`, 'yellow');
    log(`   • This wastes time waiting for an inevitable failure`, 'yellow');
    log(`   • Different Node versions may have breaking changes or missing features`, 'yellow');
    log(`\n💡 ${colors.bold}Quick solutions:${colors.reset}`, 'yellow');
    log(`   1. Switch version: ${colors.blue}nvm use ${requiredMajor}${colors.reset}`, 'yellow');
    log(`   2. Install if missing: ${colors.blue}nvm install ${requiredMajor}${colors.reset}`, 'yellow');
    log(`   3. Set as default: ${colors.blue}nvm alias default ${requiredMajor}${colors.reset}`, 'yellow');
    log(`\n🚫 ${colors.bold}Build process cancelled to save your time and prevent compatibility issues.${colors.reset}`, 'red');
    log(`   ${colors.bold}Fix the Node version above and try again.${colors.reset}`, 'red');
    return false;
  }
}
