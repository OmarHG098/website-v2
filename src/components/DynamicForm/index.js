...
const useErrorHandler = (translations) => {
  const getErrorMessage = useCallback(
    (error, context = "general") => {
      ...
      const errorMessages = {
        network:
          translations?.error_handler?.network ||
          "Connection error. Check your internet and try again.",
        timeout:
          translations?.error_handler?.timeout ||
          "Request took too long. Please try again.",
        validation:
          translations?.error_handler?.validation ||
          "Please review the entered data.",
        server:
          translations?.error_handler?.server ||
          "Server error. Try again in a few moments.",
        captcha:
          translations?.error_handler?.captcha ||
          "Verification error. Please complete the captcha.",
        general:
          translations?.error_handler?.general ||
  ...
  const validators = useMemo(
    () => ({
      first_name: (value) => {
        ...
        if (!stringValue.trim())
          return (
            translations?.validation_messages?.first_name?.required ||
            "First name is required"
          );
        if (stringValue.trim().length < 2)
          return (
            translations?.validation_messages?.first_name?.min_length ||
            "First name must be at least 2 characters"
          );
        if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(stringValue))
          return (
            translations?.validation_messages?.first_name?.invalid_format ||
      ...
      last_name: (value) => {
        ...
        if (!stringValue.trim())
          return (
            translations?.validation_messages?.last_name?.required ||
            "Last name is required"
          );
        if (stringValue.trim().length < 2)
          return (
            translations?.validation_messages?.last_name?.min_length ||
            "Last name must be at least 2 characters"
          );
        if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(stringValue))
          return (
            translations?.validation_messages?.last_name?.invalid_format ||
      ...
      email: (value) => {
        ...
        if (!stringValue.trim())
          return (
            translations?.validation_messages?.email?.required ||
            "Email is required"
          );
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(stringValue.trim()))
          return (
            translations?.validation_messages?.email?.invalid_format ||
...
const DynamicForm = ({
  ...
  const finalSubmitButtonText =
    submitButtonText || translations?.submit_button_text || "Send";
  const finalLoadingText =
    loadingText || translations?.loading_text || "Sending...";
  ...
  const createFieldHandler = useCallback(
    (fieldName) => handleFieldChange(fieldName),
    [handleFieldChange]
  );
  const createBlurHandler = useCallback(
    (fieldName) => handleFieldBlur(fieldName),
    [handleFieldBlur]
  );
        ...
        >
          ...
          <Input
            ...
            style={{
              ...
              padding: "0 16px",
              border: `2px solid ${
                hasFieldError(fieldName) ? Colors.red : "#e5e7eb"
              }`,
              borderRadius: "8px",
              fontSize: "1rem",
    ...
    async (e) => {
      ...
      try {
        ...
        const submitFunction =
          onSubmit || ((data, session) => contactUs(data, session));
    ...
    return (
      ...
      >
        <Div
          ...
          alignItems="center"
        >
          <Div fontSize="4rem" margin="0 0 24px 0">
            ✅
          </Div>
          <H3
            type="h2"
      ...
      >
        <Div padding="32px">
          ...
          >
            ...
            >
              {translations?.accessibility?.form_description ||
                "Contact form with three required fields: first name, last name and email address"}
            </div>
            ...
            {fields
              .filter(
                (field) => !["first_name", "last_name", "email"].includes(field)
              )
              .map((fieldName) => (
            ...
            <Div paddingTop="16px" display="flex" flexDirection="column">
              <Button
                ...
                aria-label={
                  status === "loading"
                    ? translations?.accessibility?.submit_loading_aria ||
                      "Sending contact form, please wait"
                    : translations?.accessibility?.submit_default_aria ||
                      "Send contact form"
                }
                style={{
                  width: "100%",
              ...
              >
                {translations?.accessibility?.submit_help_text ||
                  "Click to send your contact message. All fields marked with asterisk are required."}
              </div>
            </Div>
              ...
              >
                <Div display="flex" alignItems="center" gap="8px">
                  ...
                  <span style={{ fontFamily: "var(--font-sans)" }}>
                    {translations?.trust_indicators?.secure_confidential ||
                ...
                <Div display="flex" alignItems="center" gap="8px">
                  ...
                  <span style={{ fontFamily: "var(--font-sans)" }}>
                    {translations?.trust_indicators?.no_commitment ||
                      "No Commitment"}
                  </span>
                </Div>
                <Div display="flex" alignItems="center" gap="8px">
                  <span style={{ color: "#10b981" }}>📈</span>
                  <span style={{ fontFamily: "var(--font-sans)" }}>
                    {translations?.trust_indicators?.quick_response ||
                      "Quick Response"}
                  </span>
                </Div>
              </Div>
